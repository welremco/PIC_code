'''
This is the PIC method code for the left term is ut
Made by Hao Xu
2022.8
'''
import os
...
def translate_value_np(genome,ut,u,ux,uxx,uxxx,uxxxx,uxxxxx):
...
def translate_value_cuda(genome,ut,u,ux,uxx,uxxx,uxxxx,uxxxxx):
...
def calculate_cv(gene_translate):
    ...
    for k in range(window_num):
        ...
        Library = translate_value_np(gene_translate, Ht_n, H_n, Hx_n, Hxx_n, Hxxx_n, Hxxxx_n, Hxxxxx_n)
...
criterion='standard'    #whether has prior information, standard means no prior information
...
t_num=t.shape[0]
total=x_num*t_num
choose_validate=5000  #validate num
meta_data_num=10000   #number of meta-data
delete_num = 10
window_num=10
...
if noise_type=='Gaussian':
    ...
    un=un+noise_value
if noise_type=='Field':
    noise=pd.read_csv(fr'data/{Equation_name}/K_16.dat', header=None).values.reshape(x_num, t_num)
    un=un+noise*noise_level/100
    plt.imshow(noise)
    plt.colorbar()
    plt.show()
#save model dir
...
'''
Here, the flag controls the early termination, which means that if validate loss increase three times, the training
will be terminated. In high noise levels (especially KG equation), directly use the fixed iter_num 1000 may be better.
'''
...
gga=GGA(x_meta,t_meta,u,u_x,u_xx,u_xxx,u_t,u_tt,epi=epi,dim=1,delete_num=delete_num,creterion=criterion)
...
#========================Trianing PINN and calculate p-loss===================
...
for index in re1:
    ...
    for t in range(300):
        ...
        Htt = torch.autograd.grad(outputs=Ht.sum(), inputs=database_test, create_graph=True)[0][:, 1].reshape(test_num,
                                                                                                              1)
        ...
        Hxxx = torch.autograd.grad(outputs=Hxx.sum(), inputs=database_test, create_graph=True)[0][:, 0].reshape(
            test_num,
            1)
        Hxxxx = torch.autograd.grad(outputs=Hxxx.sum(), inputs=database_test, create_graph=True)[0][:, 0].reshape(
            test_num,
            1)
        ...
        Library=translate_value_np(gene_translate,Ht_n,H_n,Hx_n,Hxx_n,Hxxx_n,Hxxxx_n,Hxxxxx_n)
        Library_cuda=translate_value_cuda(gene_translate,Ht,prediction_test,Hx,Hxx,Hxxx,Hxxxx,Hxxxxx)
        u, d, v = np.linalg.svd(np.hstack((Ht_n, Library)), full_matrices=False)
        if t<=300:
            coef_NN = v.T[:, -1] / (v.T[:, -1][0] + 1e-8)
    ...
    error=np.sqrt(np.mean(((Net(database_test).cpu().data.numpy()-h_min)/(h_max-h_min)-(prediction_test_compare-h_min)/(h_max-h_min))**2))
...
#=====================Optimize the coefficients by PINN after the best PDE is determined==================
...
torch.save(Net.state_dict(), f'model_save/{Equation_name}/{choose}_{noise_level}_{trail_num}({noise_type})/'+f"Net_{Activation_function}_PINN.pkl")
